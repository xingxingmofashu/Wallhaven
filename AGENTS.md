...
- **Networking** — `FetchActor.shared` is an **actor** — `await` all calls. Methods: `search(filters:page:)`, `getWallpaperDetail(id:)`, `getUserSettings()`. Retries once on 429/5xx/network errors with 2s backoff; `FetchError.isRetryable` gates the retry.
