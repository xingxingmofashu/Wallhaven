...
- **网络** — `FetchActor` actor，使用 `URLSession` async/await，对临时性故障（429/5xx）自动重试
- **缓存** — 基于 `NSCache` 的 `CacheImage`（图片 1 GB，原始数据 512 MB），所有图片加载使用 `CacheAsyncImage`，支持下载去重和取消
- **持久化** — SwiftData（`FavoriteWallpaper`、`CollectionFolder`）。单个 `FavoriteWallpaper` 模型同时表示收藏（`collectionID == nil`）和收藏集成员（`collectionID` 设为文件夹 UUID）。
- **收藏集** — 纯本地：`CollectionFolder` 通过 `FavoriteWallpaper` 成员关系分组壁纸；不调用 API
...
Wallhaven/
  ...
  Models/
    ...
    Favorite/           SwiftData 模型
      Collection/       CollectionFolder
      FavoriteWallpaper
  Services/
    FetchActor.swift    网络层 actor（搜索、详情、用户设置）
